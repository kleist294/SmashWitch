...
static Vertex3D g_MeshFieldVertexs[(MESH_X + 1) * (MESH_Z + 1)];
...
{
	g_TextureID = Texture_SetTextureLoadFile("asset\\texture\\icefloor.png");
...
void MeshField_Draw(int mode,int HorinzontalCount, int VerticalCount,float height,int rotate)
{
	...
	//デバイスにワールド変換行列を設定 
	D3DXMATRIX mtxWorld,mtxT,mtxR;
	D3DXMatrixIdentity(&mtxWorld);
	D3DXMatrixTranslation(&mtxT,0.0f, height, 0.0f);

	if (rotate == 1)
	{
		D3DXMatrixRotationX(&mtxR, D3DXToRadian(-180));
		mtxWorld = mtxT * mtxR;
	}
	else
	{
